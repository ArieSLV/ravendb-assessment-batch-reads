...
namespace Raven.Client.Documents.Commands.MultiGet
{
    public class MultiGetCommand : RavenCommand<List<GetResponse>>, IDisposable
    {
        ...
        private CachedResponseSlots _localResponses;
        ...
        public override HttpRequestMessage CreateRequest(JsonOperationContext ctx, ServerNode node, out string url)
        {
            ...
            if ((_sessionInfo == null || _sessionInfo.NoCaching == false) &&
                TryServeFromLocalCopies(ctx, _requestExecutor.AggressiveCaching.Value))
            {
        ...
        private bool TryServeFromLocalCopies(JsonOperationContext ctx, AggressiveCacheOptions options)
        {
            ReleaseLocalCopies();

            bool serveLocally = options != null;
            var trackChanges = serveLocally && options.Mode == AggressiveCacheMode.TrackChanges;
            ...
            for (int i = 0; i < _commands.Count; i++)
            {
                ...
                var lease = _httpCache.Get(ctx, cacheKey, out var changeVector, out var body);
                if (lease.Item == null)
                {
                    using (lease)
                    {
                ...
                if (serveLocally && (trackChanges && lease.MightHaveBeenModified || lease.Age > options.Duration || command.CanCacheAggressively == false))
                    serveLocally = false;

                command.Headers[Constants.Headers.IfNoneMatch] = $"\"{changeVector}\"";

                _localResponses ??= new CachedResponseSlots(_commands.Count);
                _localResponses.Attach(i, lease, body);
            }

            if (serveLocally)
            {
                using (_localResponses)
                {
                    Result = new List<GetResponse>(_commands.Count);
                    for (int i = 0; i < _commands.Count; i++)
                    {
                        Result.Add(new GetResponse { Result = _localResponses?.Peek(i)?.Clone(ctx), StatusCode = HttpStatusCode.NotModified });
                    }
                }
        ...
        public override void SetResponseRaw(HttpResponseMessage response, Stream stream, JsonOperationContext context)
        {
            ...
            using (_localResponses)
            {
                ...
                foreach (var getResponse in ReadResponses(context, peepingTomStream, parser, state, buffer))
                {
                    ...
                    Result.Add(_localResponses != null && getResponse.StatusCode == HttpStatusCode.NotModified ? new GetResponse { Result = _localResponses.Peek(i)?.Clone(context), StatusCode = HttpStatusCode.NotModified } : getResponse);
        ...
        private void MaybeSetCache(GetResponse getResponse, GetRequest command, int cachedIndex)
        {
            if (getResponse.StatusCode == HttpStatusCode.NotModified)
            {
                ...
                _localResponses?.Renew(cachedIndex);
                return;
            }
        ...
        private void ReleaseLocalCopies()
        {
            //If _localResponses is not null - it means that the client approached with this multitask request to node and the request failed.
            //and now client tries to send it to another node.
            if (_localResponses != null)
            {
